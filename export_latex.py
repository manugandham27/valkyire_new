...
def write_latex(filename, is_ieee=True):
    with open(filename, 'w') as f:
        ...
        # Adding caption package for better Springer styling if needed
        if not is_ieee:
        ...
        title = "VALKYRIE-Decoder: A Decoder-Integrated Neuro-Symbolic Gating Framework for Hallucination Mitigation in LLMs"
        ...
        def write_table(data, caption, label, is_wide=False):
            env = "table*" if (is_ieee and is_wide) else "table"
            f.write(f"\\begin{{{env}}}[htbp]\n")
            f.write("\\centering\n")
        ...
        write_table(data_threshold_tbl, "DVTE query-type threshold parameters.", "threshold", is_wide=True)
        ...
        write_table(C.HYPERPARAMS, "VALKYRIE-Decoder Hyperparameter Configuration.", "hyperparams", is_wide=True)
        ...
        write_table(data_corpus_tbl, "VALKYRIE-102K Corpus composition and Reasoning Density.", "corpus", is_wide=True)
        ...
        write_table(data_comp_tbl, "Closed-domain quantitative comparison.", "comp", is_wide=True)
        ...
        write_table(data_acc_epoch_tbl, "Epoch-level accuracy with bootstrap 95\\% CI.", "epoch", is_wide=True)
        ...
        write_table(data_domain_acc, "Per-domain accuracy with 95\\% confidence intervals.", "domain", is_wide=True)
        ...
        write_table(data_abl_tbl, "Ablation study with incremental gains.", "ablation", is_wide=True)
        ...
        write_table(data_query, "Per-query-type Precision, Recall, and F1.", "perquery", is_wide=True)
        ...
        write_table(data_igcd, "IGCD conflict detection by type.", "igcd", is_wide=True)
        ...
        write_table(data_green, "Computational efficiency (FLOPs/query).", "green_ai", is_wide=True)
        ...
        write_table(data_op, "Multi-setting evaluation: closed-domain to open-domain.", "opendomain", is_wide=True)
        ...
        write_table(data_fail, "Failure case breakdown (closed-domain, n=1,000).", "fa", is_wide=True)
