...
def parse_and_convert(source_file, out_file, is_ieee):
    ...
    latex.append("\\subsection{Taxonomy of Neural Hallucination}\nFormalising the hallucination paradigm constitutes the foundational crux of modern generative deployment. Ji et al.'s comprehensive survey \\cite{ref4} established the core taxonomy defining semantic drift and domain-specific factual collapse. Further exploration by Xu et al. \\cite{ref17} provided mathematical arguments illustrating that unsupervised parametric hallucinatory behavior is structurally inevitable within autoregressive systems. Kuhn, Gal, and Farquhar \\cite{ref1} quantified this collapse through Semantic Uncertainty, capturing the breakdown of linguistic entropy as claims separate from training distributions. Zheng et al. \\cite{ref18} explicitly detailed why leading architectures like ChatGPT fall short in providing consistently truthful answers due to inherent objective disconnection, while Lin et al.'s TruthfulQA framework \\cite{ref12} exposed how models mimic human falsehoods natively.\n")
    
    latex.append("\\subsection{RAG and Retrieval Noise}\nAddressing parameter limitations, Guu et al. \\cite{ref2} introduced REALM, paving the path for closed-book reasoning augmented by dense search matrices. Lewis et al.'s seminal RAG framework \\cite{ref3} formally fused generative pathways with external Knowledge-Intensive NLP retrieval vectors. Borgeaud et al. \\cite{ref5} further validated scaling retrieval against trillions of tokens, highlighting the profound capacity for active reference checking. However, Izacard and Grave \\cite{ref6} documented that naive retrieval injections suffer severely from cross-attention noise when sourcing open-domain responses. Addressing this, Gao et al. \\cite{ref7} refined zero-shot dense retrieval mechanisms without explicit relevance labels, and Shuster et al. \\cite{ref8} demonstrated that while augmenting conversational pipelines significantly reduces base hallucination rates, strict structural gating remains fundamentally unaddressed in most parametric environments.\n")
    
    latex.append("\\subsection{Dual-Stream and Memory-Augmented Architectures}\nParallel processing streams provide deep opportunities for segregating raw generation from factual embedding checks. Zhao et al. \\cite{ref25} first demonstrated the paramount importance of Memory-Augmented Transformers capable of handling severe long-context continuous encodings. To mitigate continuous trajectory decay, Varshney et al. \\cite{ref16} employed iterative decoding streams, emphasizing that real-time architectural detection heavily outperforms post-hoc patching. Furthermore, computational pathways mapped by Peng et al. \\cite{ref9} instantiated automated external knowledge feedback loops directly influencing semantic token flow. More recently, Liu et al. \\cite{ref24} established Verify-and-Edit as a state-of-the-art knowledge-enhanced Chain-of-Thought schema, explicitly highlighting that dual-stream logical environments successfully suppress early-layer hallucinations from polluting terminal decoding pipelines.\n")
    
    latex.append("\\subsection{Epistemic Calibration and Uncertainty Quantification}\nMeasuring the boundary where models comprehend their own ignorance defines modern epistemic calibration. Kadavath et al. \\cite{ref13} pioneered explorations proving that language models possess localized hidden states correlating directly with factual validity. Expanding on internal representations, Azaria and Mitchell \\cite{ref15} employed internal activation mapping to assert that an LLM natively encodes uncertainty thresholds when reproducing unverified data. At the generation boundary, Manakul et al. \\cite{ref11} formalized SelfCheckGPT as a leading zero-resource black-box detector scaling off raw stochastic variance. Concurrently, Li et al. \\cite{ref10} critically evaluated absolute objectivity bounds within these architectures, while Ren et al. \\cite{ref14} heavily investigated raw factual drift phenomena, both validating the core hypothesis that embedded epistemic uncertainty functions effectively as a structural classification boundary.\n")
    
    latex.append("\\subsection{Green AI and Adaptive Inference}\nAs inference costs balloon exponentially, efficiency-centric dynamic evaluation protocols dominate scalable research. Chen et al. \\cite{ref19} proposed INSIDE, specifically identifying internal state covariance structures as a deterministic bypass mapping that strictly avoids multi-pass metric overhead. Following advanced efficiency paradigms, Chuang et al. \\cite{ref20} proposed DoLa, which leverages computationally cheap contrastive layer decoding, dramatically reducing hallucination rates with effectively zero overhead by utilizing existing mature representations. In their recent foundational framework, Mundler et al. \\cite{ref21} systematically tackled self-contradictory generation loops through iterative prompt verifications, effectively exposing the critical need for single-pass architectural corrections. Ultimately, Huang et al. \\cite{ref22} and Wang et al. \\cite{ref23} both concluded in large-scale taxonomical reviews that the future of hallucination mitigation fundamentally requires end-to-end structurally integrated gating without relying exponentially on recursive inference layers.\n")
